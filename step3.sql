...
SELECT p.title AS program_title, s.season_number, e.episode_number, e.title AS episode_title, ev.view_count
FROM episodes e
...
SELECT c.name AS channel_name, cp.start_time, cp.end_time, s.season_number, e.episode_number, e.title AS episode_title, e.detail AS episode_detail
...
SELECT cp.start_time, cp.end_time, s.season_number, e.episode_number, e.title AS episode_title, e.detail AS episode_detail
FROM channel_programs cp
JOIN channels c ON cp.channel_id = c.id
JOIN programs p ON cp.program_id = p.id
JOIN seasons s ON p.id = s.program_id
JOIN episodes e ON s.id = e.season_id
WHERE c.name = 'ドラマチャンネル' AND DATE(cp.start_time) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 WEEK)
ORDER BY cp.start_time;
